'''
Compare entity-level and word-level predictions on BMASS analogy data, to check for
...
from lib import eval_mode, data_mode
from parsers import BMASS
...
def writeAnalysis(analogies, outf, key_order):
    ...
    for (rel, anlgs) in analogies.items():
        ...
        for (_, output_info) in anlgs:
            (ent_saw_it, ent_result, _) = output_info.get(data_mode.Concept, (False, None, None))
            (wrd_saw_it, wrd_result, _) = output_info.get(data_mode.String, (False, None, None))
...
if __name__ == '__main__':
    ...
    analogies = BMASS.read(options.analogy_file, eval_mode.MULTI_ANSWER, strings_only=False, cuis_only=False)
